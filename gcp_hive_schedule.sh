...
gcloud dataproc jobs submit hive --cluster=cluster-iz-dplr --region us-central1 -e "load data inpath '/user/hduser/project/txns' overwrite into table transactions" &> /tmp/gcp_hive_schedule.log
if [ $? -ne 0 ]
then
...
loaddt=$(date '+%Y-%m-%d')
gcloud dataproc jobs submit hive --cluster=cluster-iz-dplr --region us-central1 --file=hdfs:///user/hduser/project/cust_etl.hql --continue-on-failure \
